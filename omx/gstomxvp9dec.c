...
#define GST_OMX_VP9_OUTPUT_WIDTH_MAX 4096
#define GST_OMX_VP9_OUTPUT_WIDTH_MIN 240
#define GST_OMX_VP9_OUTPUT_HEIGHT_MAX 2160
#define GST_OMX_VP9_OUTPUT_HEIGHT_MIN 135
#endif
...
static gboolean gst_omx_vp9_dec_set_format (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state);
#ifdef __LINUX_MEDIA_NAS__
static void gst_omx_vp9_dec_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_omx_vp9_dec_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static void gst_omx_vp9_dec_set_scaling (GstOMXVideoDec * self);
#endif
...
#ifdef __LINUX_MEDIA_NAS__
  gobject_class->set_property = gst_omx_vp9_dec_set_property;
  gobject_class->get_property = gst_omx_vp9_dec_get_property;
  g_object_class_install_property (gobject_class, PROP_WIDTH,
      g_param_spec_uint ("width", "width",
          "video width of decode output port",
          GST_OMX_VP9_OUTPUT_WIDTH_MIN, GST_OMX_VP9_OUTPUT_WIDTH_MAX,
          GST_OMX_VP9_OUTPUT_WIDTH_MIN,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, PROP_HEIGHT,
      g_param_spec_uint ("height", "height",
          "video height of decode output port",
          GST_OMX_VP9_OUTPUT_HEIGHT_MIN, GST_OMX_VP9_OUTPUT_HEIGHT_MAX,
          GST_OMX_VP9_OUTPUT_HEIGHT_MIN,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    GstVideoCodecState * state)
{
...
#ifdef __LINUX_MEDIA_NAS__
  GstOMXVP9Dec *self = GST_OMX_VP9_DEC (dec);
  ...
  gst_omx_vp9_dec_set_scaling (dec);
#endif
...
#ifdef __LINUX_MEDIA_NAS__
static void
gst_omx_vp9_dec_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstOMXVP9Dec *self = GST_OMX_VP9_DEC (object);
  ...
  switch (prop_id) {
    case PROP_WIDTH:
    {
      ...
      if (guVal >= GST_OMX_VP9_OUTPUT_WIDTH_MIN
          && guVal <= GST_OMX_VP9_OUTPUT_WIDTH_MAX) {
        self->width = guVal;
      } else {
    ...
    case PROP_HEIGHT:
    {
      ...
      if (guVal >= GST_OMX_VP9_OUTPUT_HEIGHT_MIN
          && guVal <= GST_OMX_VP9_OUTPUT_HEIGHT_MAX) {
        self->height = guVal;
      } else {
        GST_FIXME_OBJECT (self, "[%s] Not in range\n", __FUNCTION__);
      }
...
static void
gst_omx_vp9_dec_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstOMXVP9Dec *self = GST_OMX_VP9_DEC (object);
...
static void
gst_omx_vp9_dec_set_scaling (GstOMXVideoDec * dec)
{
  GstOMXVP9Dec *self = GST_OMX_VP9_DEC (dec);
  guint32 param[4];
