...
#define GST_OMX_H265_OUTPUT_WIDTH_MAX 4096
#define GST_OMX_H265_OUTPUT_WIDTH_MIN 240
#define GST_OMX_H265_OUTPUT_HEIGHT_MAX 2160
#define GST_OMX_H265_OUTPUT_HEIGHT_MIN 135
#endif
...
static gboolean gst_omx_h265_dec_set_format (GstOMXVideoDec * dec,
    GstOMXPort * port, GstVideoCodecState * state);
#ifdef __LINUX_MEDIA_NAS__
static void gst_omx_h265_dec_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_omx_h265_dec_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);
static void gst_omx_h265_dec_set_scaling (GstOMXVideoDec * self);
#endif
...
gst_omx_h265_dec_class_init (GstOMXH265DecClass * klass)
{
  ...
  gst_element_class_set_static_metadata (element_class,
      ...
      "Codec/Decoder/Video", "Decode H.265 video streams", "Realtek");
  ...
  g_object_class_install_property (gobject_class, PROP_WIDTH,
      g_param_spec_uint ("width", "width",
          ...
          GST_OMX_H265_OUTPUT_WIDTH_MIN, GST_OMX_H265_OUTPUT_WIDTH_MAX,
          GST_OMX_H265_OUTPUT_WIDTH_MIN,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));

  g_object_class_install_property (gobject_class, PROP_HEIGHT,
      g_param_spec_uint ("height", "height",
          "video height of decode output port",
          GST_OMX_H265_OUTPUT_HEIGHT_MIN, GST_OMX_H265_OUTPUT_HEIGHT_MAX,
          GST_OMX_H265_OUTPUT_HEIGHT_MIN,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS));
    ...
    GstVideoCodecState * state)
{
  ...
  gst_omx_h265_dec_set_scaling (dec);
#endif
...
#ifdef __LINUX_MEDIA_NAS__
static void
gst_omx_h265_dec_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstOMXH265Dec *self = GST_OMX_H265_DEC (object);
  ...
  switch (prop_id) {
    case PROP_WIDTH:
    {
      ...
      if (guVal >= GST_OMX_H265_OUTPUT_WIDTH_MIN
          && guVal <= GST_OMX_H265_OUTPUT_WIDTH_MAX) {
        self->width = guVal;
      } else {
    ...
    case PROP_HEIGHT:
    {
      ...
      if (guVal >= GST_OMX_H265_OUTPUT_HEIGHT_MIN
          && guVal <= GST_OMX_H265_OUTPUT_HEIGHT_MAX) {
        self->height = guVal;
      } else {
        GST_FIXME_OBJECT (self, "[%s] Not in range\n", __FUNCTION__);
      }
...
static void
gst_omx_h265_dec_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec)
{
  GstOMXH265Dec *self = GST_OMX_H265_DEC (object);
...
static void
gst_omx_h265_dec_set_scaling (GstOMXVideoDec * dec)
{
  GstOMXH265Dec *self = GST_OMX_H265_DEC (dec);
  guint32 param[4];
