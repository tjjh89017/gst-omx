...
static const GGetTypeFunction types[] = {
  ...
  gst_omx_amr_dec_get_type, gst_omx_avs_dec_get_type, gst_omx_jpeg_enc_get_type,
#ifdef HAVE_VP8
...
static const struct TypeOffest base_types[] = {
  ...
  {gst_omx_image_enc_get_type, G_STRUCT_OFFSET (GstOMXImageEncClass, cdata)},
};
...
/* comp->lock must be unlocked while calling this */
OMX_ERRORTYPE
gst_omx_component_get_extension_index (GstOMXComponent * comp, OMX_STRING index,
    gpointer param)
{
  OMX_ERRORTYPE err;
  ...
  GST_DEBUG_OBJECT (comp->parent, "Setting %s parameter at index 0x%08lx",
      comp->name, (unsigned long) index);
  err = OMX_GetExtensionIndex (comp->handle, index, param);
  GST_DEBUG_OBJECT (comp->parent, "Set %s parameter at index 0x%08lx: %s "
      "(0x%08x)", comp->name, (unsigned long) index,
      gst_omx_error_to_string (err), err);
...
plugin_init (GstPlugin * plugin)
{
  ...
  if (env_config_dir)
	...
	config_dirs[i++] = (gchar *) "/var/packages/VideoStation/target/etc";
#else
